...
describe('PromptManager', () => {
  ...
  it('uses qwen profile overrides from environment variable', () => {
    ...
    expect((config.options as Record<string, number>).num_ctx).toBeGreaterThan(4000);
  });
  ...
  it('merges custom mode overrides', () => {
    ...
    const config = manager.getModeConfig('accurate');
    expect((config.options as Record<string, number>).temperature).toBe(0.5);
    expect((config.options as Record<string, number>).num_ctx).toBeGreaterThan(0);
  });
