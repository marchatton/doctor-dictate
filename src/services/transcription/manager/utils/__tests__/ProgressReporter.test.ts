...
describe('ProgressReporter', () => {
  it('emits events through provided emitter', () => {
    ...
    expect(emitter.emit).toHaveBeenCalledWith('stage', expect.objectContaining({ stage: 'vad', status: 'progress' }));
    expect(emitter.emit).toHaveBeenCalledWith('chunk', expect.objectContaining({ current: 1, total: 5 }));
