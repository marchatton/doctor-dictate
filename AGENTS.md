...
Run `pnpm run dev` for combined Vite + Electron development and `pnpm start` to replay the packaged shell. `pnpm run react:build` produces the renderer bundle; `pnpm run build` extends it with `electron-builder`. Begin each change with a failing Jest spec (`pnpm test` or `pnpm run test:watch`), drive it green, then refactor. Maintain coverage through `pnpm run test:coverage` and mirror CI with `pnpm run test:ci`. Lint via `pnpm run lint` or `pnpm run lint:fix`, and regenerate prompts after edits using `pnpm run build-prompt`.
