...
**LOCAL-FIRST, PRIVACY-FOCUSED FOR MEDICAL DATA.** This application processes sensitive patient data entirely on-device. No external APIs for transcription or formatting. All processing happens locally using Whisper and Ollama.
...
- Local-first architecture (no cloud dependencies for medical data)
...
- **Medical Formatting**: Ollama (local)
...
- `./run-tests.sh` - Run tests with prerequisite checking
...
test/
  medical-prompt.test.js // Prompt formatting tests
```
...
- **Never lose content**: 100% content preservation from dictation
- **Preserve exact wording**: Keep medication names and dosing as dictated
- **Content verification**: Implement 80% coverage threshold checks
- **Template compliance**: Follow medical note structure exactly

```typescript
// Content verification example
const verificationResult = contentVerifier.verifyContent(input, output);
if (verificationResult.coverage < 0.8) {
  // Re-inject missing content or retry formatting
}
```

### Local Service Checks

Always verify local services before processing:

```typescript
// Always check local services
const isOllamaAvailable = await checkOllamaConnection();
if (!isOllamaAvailable) {
  // Fallback to simple formatting or notify user
}
...
- **Templates define structure**: All formatting rules come from templates
- **No hardcoded sections**: Use `templates/format/*.json` for definitions
- **Smart section detection**: 8 confidence-scored rules for finding sections
- **Single sources of truth**:
  - Medical corrections: `data/medical-dictionary.js`
  - Dictation commands: `data/dictation-commands.js`
  - Dosing patterns: `data/dosing-patterns.js`

### Audio Pipeline

```
Audio Input → Whisper Transcription → Medical Formatting (Ollama) → Verification → Output
```

- Fast mode: For quick notes during patient interaction
- Accurate mode: For final documentation
...
ipcMain.handle('transcribe-audio', async (event, audioPath) => {
  return await whisperTranscriber.transcribe(audioPath);
});
...
// Avoid: External API calls for medical data processing
const transcript = await callCloudAPI(audio); // Never!

// Avoid: Losing medical content
if (text.length > 1000) text = text.substring(0, 1000); // Never truncate medical notes!

// Avoid: Hardcoding medical rules
if (section === 'meds') { // Use templates!
  format = 'numbered';
}

// Avoid: Assuming service availability
const result = await ollama.format(text); // Always check first!
```

## Updates Log

When you discover something that would have been helpful to know at the start of a task, add it here:

- **Audio Format Conversion**: Whisper.cpp requires WAV format. Always convert m4a/mp3 to WAV before processing.
- **Ollama Timeout**: Set 60-second timeout for Ollama requests. Medical notes can be long and complex.
- **Content Verification**: Always verify >80% content coverage after formatting to catch missing sentences.
- **MediaRecorder MIME Types**: Browser MediaRecorder needs explicit MIME type detection for cross-browser compatibility.
- **Section Detection**: Look for patterns like "new paragraph + short phrase + colon" to detect sections.
- **Template Paths**: Templates are in `src/templates/format/` not `docs/templates/`.
- **Whisper Models**: Must be downloaded to `~/.whisper-cpp/models/` before use.
- **FFmpeg Required**: Audio conversion from m4a to WAV requires FFmpeg installed (`brew install ffmpeg`).
- **Node.js Fetch**: Ollama tests fail in Node.js environment due to missing fetch API - this is expected.
...
For this medical application specifically, remember that patient data never leaves the device, 100% content preservation is mandatory, and all processing must happen locally through Whisper and Ollama.