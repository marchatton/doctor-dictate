/**
 * Test medical formatting with real Whisper transcription using llama3.2
 */
...
async function testRealTranscription() {
    console.log('🏥 End-to-End Test with Real Transcription (llama3.2)');
    ...
    // Force use of llama3.2
    const formatter = new OllamaFormatter();
    formatter.model = 'llama3.2:latest';  // Override model selection
    ...
    if (result.success) {
        ...
        const formattedPath = 'docs/sample-data/mock-recording-formatted-llama.md';
        ...
        const qualityChecks = [
            ...
            { 
                ...
                check: /\d+\s*mg|milligrams/i.test(result.formatted),
            ...
            { 
                name: 'Medication names clarified', 
                check: /Jornay|Lexapro/i.test(result.formatted),
                details: 'Journey -> Jornay, Luxapro -> Lexapro'
            }
        ];
